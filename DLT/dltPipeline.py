...
def stage_bookings():
    df = spark.readStream.format("delta") \
        .load("/Volumes/flightsproject/bronze/bronzevolume/bookings/data/")
    return df
...
def trans_flights():
    df = spark.readStream.format("delta") \
            .load("/Volumes/flightsproject/bronze/bronzevolume/flights/data/")
...
def trans_flights():
    df = spark.readStream.format("delta") \
            .load("/Volumes/flightsproject/bronze/bronzevolume/customers/data/")
    df = df.drop("_rescued_data") \
            .withColumn("modifiedDate", current_timestamp())
    return df
...
def trans_flights():
    df = spark.readStream.format("delta") \
            .load("/Volumes/flightsproject/bronze/bronzevolume/airports/data/")
    df = df.drop("_rescued_data") \
            .withColumn("modifiedDate", current_timestamp())
    return df
